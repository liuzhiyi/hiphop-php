...
void ScalarExpression::outputCPPString(CodeGenerator &cg, AnalysisResultPtr ar) {
  switch (m_type) {
  case T_CONSTANT_ENCAPSED_STRING:
  case T_ENCAPSED_AND_WHITESPACE:
    ASSERT(m_quoted); // fall through
  case T_STRING: {
    ...
    if (m_quoted) {
      ...
      if (hasEmbeddedNull) {
        char length[20];
        snprintf(length, sizeof(length), "%zd", m_value.length());
        bool constant =
